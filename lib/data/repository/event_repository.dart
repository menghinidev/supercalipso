...
class EventRepository {
  ...
  Stream<List<TeamEvent>> getTeamEventsChanges({required String teamId}) =>
      eventsChanges.map((event) => event.where((element) => element.team.id == teamId).toList());

  Stream<TeamEvent> getEventChanges({required String eventId}) =>
      eventsChanges.mapNotNull((event) => event.getWhere((element) => element.team.id == eventId));
  ...
  Future<Response> createEvent({
    ...
    var event = await provider.createTeamEvent(team: team, name: name, startTime: startTime, duration: duration);
